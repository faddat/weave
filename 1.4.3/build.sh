...
mkdir weave
cd weave
git clone --branch v$VERSION https://github.com/weaveworks/weave.git .

sed -i 's|^RUN_FLAGS=-ti$|RUN_FLAGS=|' Makefile
sed -i 's|^DOCKERHUB_USER=weaveworks$|DOCKERHUB_USER=armhfbuild|' Makefile
sed -i 's|^WEAVEEXEC_DOCKER_VERSION=.*$|WEAVEEXEC_DOCKER_VERSION=10.12.3|' Makefile
...
sed -i 's|^FROM golang:.*$|FROM armhfbuild/golang:1.7.1|' build/Dockerfile
